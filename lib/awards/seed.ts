/* ============================================================
   ...
   ============================================================ */
import type { EmploymentType, ShiftSession } from "./higa";
import { floorHourly, type Level } from "./rates";

/* ---------- what people are paid ----------

   These used to be hand-typed numbers — 31.23, 28.87, 25.80 — and they reached
   the missed-break penalty estimate, the payroll report's dollar total and the
   marketplace's "above award" badge as though somebody had checked them.
   Nobody had. They are now derived from the award, so the demo's money is the
   same money the rest of the system reasons about.

   The CLASSIFICATION table is authored, exactly as a venue would have to author
   it: which level a job sits at is a legal position about duties, not something
   a role string can be trusted to imply. See suggestedLevel() in rates.ts. */
const CLASSIFICATION: Record<string, Level> = {
  Bartender: 2,
  "Bar Attendant": 2,
  "Gaming Attendant": 2,
  "Wait Staff": 2,
...
/**
 * The award's ordinary hourly rate for a demo role, in dollars.
 *
 * Employment type is passed rather than looked up so it cannot drift from the
 * employmentType on the same session — a casual priced at the permanent rate
 * would understate every penalty computed from it.
 */
function rate(role: string, employment: EmploymentType): number {
  const level = CLASSIFICATION[role];
  if (level == null) throw new Error(`No classification recorded for ${role}`);
  return floorHourly(level, employment) / 100;
}
...
export const DEMO_SESSIONS: ShiftSession[] = [
  ...
  { userId: "w:darie-roberts", name: "Darie Roberts", role: "Bartender", siteName: "Brightwater Hotel", employmentType: "casual", ordinaryHourlyRate: rate("Bartender", "casual"),
    clockIn: at(6, 20), plannedEnd: at(6, 20) + 8 * H, breaks: [] },
  // Due soon — 5h25 in, no meal, 8h roster.
  { userId: "w:liam-obrien", name: "Liam O'Brien", role: "Barback", siteName: "Brightwater Hotel", employmentType: "casual", ordinaryHourlyRate: rate("Barback", "casual"),
    clockIn: at(5, 25), plannedEnd: at(5, 25) + 8 * H, breaks: [] },
  // cl 16.7(a) — meal at 2h30, now 7h45 in, 10h roster; also owed a paid rest.
  { userId: "w:hassan-ali", name: "Hassan Ali", role: "Head Chef", siteName: "Brightwater Hotel", employmentType: "full_time", ordinaryHourlyRate: rate("Head Chef", "full_time"),
    clockIn: at(7, 45), plannedEnd: at(7, 45) + 10 * H,
    breaks: [{ kind: "meal", start: at(7, 45) + 2.5 * H, end: at(7, 45) + 3 * H }] },
  // On meal break right now.
  { userId: "w:mitch-egan", name: "Mitch Egan", role: "Gaming Attendant", siteName: "Brightwater Gaming Room", employmentType: "casual", ordinaryHourlyRate: rate("Gaming Attendant", "casual"),
    clockIn: at(4, 10), plannedEnd: at(4, 10) + 7 * H,
    breaks: [{ kind: "meal", start: at(0, 12), end: null }] },
  // Window open — 3h in, 8h roster.
  { userId: "w:priya-sharma", name: "Priya Sharma", role: "Events Coordinator", siteName: "Werribee Park Wedding", employmentType: "part_time", ordinaryHourlyRate: rate("Events Coordinator", "part_time"),
    clockIn: at(3, 5), plannedEnd: at(3, 5) + 8 * H, breaks: [] },
  // Too early — 1h in.
  { userId: "w:jake-morrison", name: "Jake Morrison", role: "Bar Attendant", siteName: "Northside Tavern", employmentType: "casual", ordinaryHourlyRate: rate("Bar Attendant", "casual"),
    clockIn: at(1, 0), plannedEnd: at(1, 0) + 8 * H, breaks: [] },
  // 11h roster — meal + one rest done, second rest owed.
  { userId: "w:leanne-vidal", name: "Leanne Vidal", role: "Duty Manager", siteName: "Brightwater Hotel", employmentType: "full_time", ordinaryHourlyRate: rate("Duty Manager", "full_time"),
  ...
  { userId: "w:michael-tan", name: "Michael Tan", role: "Wait Staff", siteName: "Docklands Corporate Lunch", employmentType: "casual", ordinaryHourlyRate: rate("Wait Staff", "casual"),
    clockIn: at(2, 0), plannedEnd: at(2, 0) + 4 * H, breaks: [] },
  // No roster link, 5h15 elapsed — elective zone.
  { userId: "w:aaron-patel", name: "Aaron Patel", role: "Bartender", siteName: "Quayside Bar & Kitchen", employmentType: "casual", ordinaryHourlyRate: rate("Bartender", "casual"),
...
const clean = (
  // not `rate`: that is the module-level lookup, and shadowing it here would
  // make the call sites read as if they were passing something else
  userId: string, name: string, role: string, siteName: string, hourlyRate: number,
  d: number, startH: number, lenH: number,
): ShiftSession => ({
  userId, name, role, siteName, employmentType: "casual", ordinaryHourlyRate: hourlyRate,
...
export const DEMO_WEEK_SESSIONS: ShiftSession[] = [
  ...
  clean("w:priya-sharma", "Priya Sharma", "Events Coordinator", "Brightwater Hotel", rate("Events Coordinator", "casual"), 0, 10, 8),
  clean("w:priya-sharma", "Priya Sharma", "Events Coordinator", "Brightwater Hotel", rate("Events Coordinator", "casual"), 1, 10, 8),
  clean("w:michael-tan", "Michael Tan", "Wait Staff", "Brightwater Hotel", rate("Wait Staff", "casual"), 2, 11, 8),
  clean("w:jake-morrison", "Jake Morrison", "Bar Attendant", "Northside Tavern", rate("Bar Attendant", "casual"), 3, 16, 8),
  clean("w:mitch-egan", "Mitch Egan", "Gaming Attendant", "Brightwater Gaming Room", rate("Gaming Attendant", "casual"), 4, 9, 9),
  ...
  { userId: "w:darie-roberts", name: "Darie Roberts", role: "Bartender", siteName: "Brightwater Hotel",
    employmentType: "casual", ordinaryHourlyRate: rate("Bartender", "casual"),
  ...
  { userId: "w:leanne-vidal", name: "Leanne Vidal", role: "Duty Manager", siteName: "Brightwater Hotel",
    employmentType: "full_time", ordinaryHourlyRate: rate("Duty Manager", "full_time"),
  ...
  { userId: "w:liam-obrien", name: "Liam O'Brien", role: "Barback", siteName: "Brightwater Hotel",
    employmentType: "casual", ordinaryHourlyRate: rate("Barback", "casual"),
  ...
  { userId: "w:sophie-nguyen", name: "Sophie Nguyen", role: "Venue Manager", siteName: "Brightwater Hotel",
    employmentType: "full_time", ordinaryHourlyRate: rate("Venue Manager", "full_time"),
