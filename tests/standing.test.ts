...
describe("the seeded demo beats", () => {
  it("has Michael Tan needing action — his RSA is revoked", () => {
...
describe("what counts as which state", () => {
  ...
  it("counts everything the person holds, problems included", () => {
    const s = standing("Michael Tan");
    expect(s.held.length).toBeGreaterThan(s.problems.length);
  });

  it("treats revoked as blocking regardless of the expiry date", () => {
  ...
  it("uses the engine's own warning window, not a second number", () => {
  ...
  it("reports null days for a credential that does not expire", () => {
  ...
  it("is current for somebody holding nothing at all", () => {
...
describe("it answers as at a date, not as at today", () => {
  it("turns an expiring credential into an expired one by moving the date", () => {
  ...
  it("shows a credential as current well before its warning window", () => {
...
describe("daysBetween", () => {
  ...
  it("narrows a timestamp to its day, so the answer does not drift", () => {
