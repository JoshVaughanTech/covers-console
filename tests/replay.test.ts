...
describe("an empty log", () => {
  it("gives back the seed unchanged", () => {
...
describe("replaying what happened", () => {
  ...
  it("restores an assignment and the seat it fills", () => {
  ...
  it("marks a posting filled once its seats are taken", () => {
    const log = chain(
  ...
  it("restores a decline with the reason the manager gave", () => {
  ...
  it("restores a posting that did not exist in the seed", () => {
  ...
  it("replays a claim on a posting that itself came from the log", () => {
...
describe("replaying is lossless and repeatable", () => {
  ...
  it("gives the same board every time — a reload is not a dice roll", () => {
    expect(replayPostings(POSTINGS, log())).toEqual(replayPostings(POSTINGS, log()));
  });

  it("does not mutate the seed it folds over", () => {
    const before = structuredClone(POSTINGS);
    replayPostings(POSTINGS, log());
    expect(POSTINGS).toEqual(before);
  });

  it("is idempotent per event — a replayed claim is not a second claim", () => {
  ...
  it("does not assign the same person twice", () => {
  ...
  it("does not add a posted shift twice", () => {
...
describe("which events belong to the board", () => {
  it("claims that a shift event is ours", () => {
    const [e] = chain(claimed("sp-2041-bar", did("Darie Roberts")));
    expect(isShiftEvent(e)).toBe(true);
  });

  it("recognises a decline by its shape, since it writes as a plain decision", () => {
    const [e] = chain(declined("sp-2041-wait", did("Mitch Egan"), "no"));
    expect(isShiftEvent(e)).toBe(true);
  });

  it("ignores an unrelated decision", () => {
  ...
  it("ignores a credential revocation", () => {
    const [e] = chain({
  ...
  it("leaves the board untouched when the log holds nothing of ours", () => {
    const log = chain({
...
describe("events that cannot be applied", () => {
  it("skips a posted event carrying no posting, rather than failing the board", () => {
    const log = chain({
      type: "shift.posted",
      at: AT,
      actor: "Emma Taylor",
  ...
  it("ignores an event naming a posting that does not exist", () => {
    const log = chain(claimed("sp-does-not-exist", did("Darie Roberts")));
    expect(replayPostings(POSTINGS, log)).toEqual(POSTINGS);
  });

  it("ignores a claim with no subject", () => {
    const log = chain({
      type: "shift.claimed",
      at: AT,
      actor: "someone",
